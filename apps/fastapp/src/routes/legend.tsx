...
const List = observer(() => {
  const update = (id: number) => {
    const index = list.get().findIndex((item) => item.id === id);
    list[index].text.set('updated');
  };
  return (
    <For each={list} optimized>
      {(item$) => {
        console.log('RENDER ITEM', item$.get());
        return (
          <div className="flex flex-row">
            <div>{item$.text.get()}</div>
            <button onClick={() => update(item$.id.get())}>Update</button>
          </div>
        );
      }}
    </For>
