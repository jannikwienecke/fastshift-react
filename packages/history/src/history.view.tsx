...
import { HistoryViewDataType } from './history.types';
...
export const uiViewConfig = makeViewFieldsConfig<HistoryViewDataType>(
  'history',
  {
    fields: {
      entityId: {
          ...
          detailValue: ({ data }) => {
            return (
              <BubbleItem
                ...
                icon={() => <ViewIconOf viewName="owner" />}
              />
            );
          },
          list: ({ data }) => {
...
export const HistoryMainPage = observer(() => {
  return (
...
export const HistoryDetailPage = observer(() => {
  return (
    <>
      <DefaultDetailViewTemplate
        detailOptions={{
...
export const HistoryOverviewPage = observer(() => {
  const { t } = useTranslation();
